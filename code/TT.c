...
void initTT(){
  ...
  for(i=0;i<TTDIM;i++){
    ...
    pthread_mutex_init(&TABLE_LOCK_WHITE[i],(void *) 0);
    pthread_mutex_init(&TABLE_LOCK_BLACK[i],(void *) 0);
  }
}



// Add a colmpetly evaluate node NB: you must not call it if the node is a cut-node
...
// Know if the board is already been evaluate whne min or max start
...
void printEntry(Entry entry){
  ...
  printf(" | ");
  printf(" depth: %d | value: %d | type: %d |\n" , entry.depth, entry.value, entry.type );
...
void addFirstWhite(Hash hash, int value,int depth,Move bestMove){
  int h = hash & BITMASK;
  pthread_mutex_lock(&TABLE_LOCK_WHITE[h]); 
  if(whiteTT[h].value<value && whiteTT[h].depth<=depth){
     whiteTT[h].hash = hash;
    whiteTT[h].value = value;
    whiteTT[h].depth = depth;
    whiteTT[h].bestMove = bestMove;
    whiteTT[h].type=UPPER_ENTRY;
  }
  pthread_mutex_unlock(&TABLE_LOCK_WHITE[h]);
}
void addFirstBlack(Hash hash, int value,int depth,Move bestMove){
    int h = hash & BITMASK;
  pthread_mutex_lock(&TABLE_LOCK_BLACK[h]); 
  if(blackTT[h].value>value && blackTT[h].depth<=depth ){
     blackTT[h].hash = hash;
     blackTT[h].value = value;
     blackTT[h].depth = depth;
     blackTT[h].bestMove = bestMove;
     blackTT[h].type=LOWER_ENTRY;
  }
  pthread_mutex_unlock(&TABLE_LOCK_BLACK[h]);
