...
inline Hash stringHash(BoardState* board) {
    Hash hash = FIRSTH;
    int i;
    for (i = 0; i < 9; i++) {
...
//2-zobrist puro
inline Hash zobrist(BoardState* board){
	int i,j;
	int h=31;
	for(i=0; i< 9; i++)
		for(j=0;j<9;j++)
			h=h ^ table[i*9+j][GETCELLSTATE(board->Board,i,j)];
	return h;
}

//3-zobrist forse un po' più veloce(ma non credo troppo) 
inline Hash fastZobrist(BoardState* board){
	int i,j;
	int h=31;
	int cell;
	for(i=0; i< 9; i++){
		for(j=0;j<9;j++){
			cell=GETCELLSTATE(board->Board,i,j);
			if(cell!=0)
...
inline inline Hash fastHash(BoardState* board){
	int i;
	Hash hash=31;
	for(i=0;i<9;i++){
