...
void gameLoop(int playerColor, int maxTime, int numThreads) {
	...
	setRelativeHistoryValueTable();	//SORT
	...
	while(read(sd, stateString, 1) > 0) {       // Ricezione stato
		...
		if(turn == BLACK_TURN && playerColor == BLACK) {
			...
			*/
			pushPlayed(state, numThreads);	//add state to the branch
			printf("\nBest move: ");
			printMove(move);
			printf("\n");
			resetStack(numThreads);
			pushPlayed(moveBlack(state,move),numThreads);	//add the board to the branch
			...
			*/
			...
			printf("\n");
			resetStack(numThreads);
			pushPlayed(moveWhite(state,move),numThreads);	//add the board to the branch
			// Invio mossa
			sendMove(sd, move, playerColor);
		}
